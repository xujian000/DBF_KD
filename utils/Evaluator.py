...
def cosine_similarity(x, y):
    ...
    y = F.normalize(y, p=2, dim=1)  # 对 y 进行 L2 归一化
    return torch.sum(x * y, dim=1)  # 计算余弦相似度


# 威尔逊相似度
def pearson_correlation(x, y):
    """
    计算两个特征向量或特征矩阵之间的皮尔逊相关系数
    x: Tensor of shape (batch_size, feature_dim)
    y: Tensor of shape (batch_size, feature_dim)
    return: Tensor of shape (batch_size,)
    """
    ...
    return cov / (std_x * std_y + 1e-6)  # 加1e-6防止除以零


# 欧氏距离
def euclidean_distance(x, y):
    """
    计算两个特征向量或特征矩阵之间的欧氏距离
    x: Tensor of shape (batch_size, feature_dim)
    y: Tensor of shape (batch_size, feature_dim)
    return: Tensor of shape (batch_size,)
    """
...
def average_similarity(x, y, metric):
    """
    计算所有样本相似度的平均值。
    metric: str, 指定计算相似度的度量方式 (cosine, pearson, euclidean)
    """
    if metric == "cosine":
    ...
    return torch.mean(similarity).item()  # 返回标量值
