...
for dataset_name in ["MRI_CT", "MRI_PET", "MRI_SPECT"]:
    ...
    for ckpt_path in [CDDFuse_path]:
        ...
        BaseFuseLayer = nn.DataParallel(BaseFeatureExtraction(dim=64, num_heads=8)).to(
            device
        )
        DetailFuseLayer = nn.DataParallel(DetailFeatureExtraction(num_layers=1)).to(
            device
        )
        ...
        with torch.no_grad():
            for img_name in os.listdir(
                os.path.join(test_folder, dataset_name.split("_")[0])
            ):
                data_IR = (
                    image_read_cv2(
                        os.path.join(test_folder, dataset_name.split("_")[1], img_name),
                        mode="GRAY",
                ...
                data_VIS = (
                    image_read_cv2(
                        os.path.join(test_folder, dataset_name.split("_")[0], img_name),
                        mode="GRAY",
                    )[np.newaxis, np.newaxis, ...]
                    / 255.0
                )

                data_IR, data_VIS = torch.FloatTensor(data_IR), torch.FloatTensor(
                    data_VIS
                )
        ...
        for img_name in os.listdir(
            os.path.join(ori_img_folder, dataset_name.split("_")[0])
        ):
            ir = image_read_cv2(
                os.path.join(ori_img_folder, dataset_name.split("_")[1], img_name),
                "GRAY",
            )
            vi = image_read_cv2(
                os.path.join(ori_img_folder, dataset_name.split("_")[0], img_name),
                "GRAY",
            )
            fi = image_read_cv2(
